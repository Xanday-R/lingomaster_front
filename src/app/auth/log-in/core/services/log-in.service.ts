...
export class LogInService {
  ...
  readonly logIn$ = this.askLogIn$.pipe(switchMap(({email, password}) => this.http.post<ResponseFromServer>(`${environment.apiUrl}/auth/log-in`, {email, password}, this.headers()).pipe(catchError((err) => of(err.error as ResponseFromServer)))));
